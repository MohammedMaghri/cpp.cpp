...
/*   Updated: 2024/09/27 16:28:16 by mmaghri          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void PrintAswitch(std::string value){
    ...
    if (value.length() > 1) std::cout << "Char     : Invalid Convertion !" << std::endl ; 
    ...
    //To check Float 
    if (searchHomeMany(value, ".") > 0 && searchHomeMany(value, "f") == 0 && ifNumber(value) == 0){
        try {
            float tomuch = std::stof(value.c_str());
        ...
        try {
            float tomuch = static_cast<float>(std::stof(value.c_str()));
            if (searchHomeMany(value, "f") > 0){
    ...
    //To Doble 
     if (searchHomeMany(value, ".") > 0 && ifNumber(value) == 0){
        try {
        ...
        try {
            ...
            std::cout << "Double    : No Convertion Possible !" << std::endl;
        } catch (std::out_of_range &e){
            std::cout << "Double    : Out Of range !" << std::endl ;
        }
    }
